...
public class DeathBodyManager {
    ...
    public DeathBodyManager(RealmsCraft plugin) {
    ...
    private ItemStack[] getPlayerStealables(Player p) {
        ...
        for(int i = 0; i < inv.getSize(); i++) {
            ItemStack itemToSteal = inv.getItem(i);
            if(itemToSteal == null) continue;
            if(itemToSteal.getItemMeta() == null) continue;
            if(itemToSteal.getItemMeta().getLore() == null) continue;
            if(itemToSteal.getItemMeta().getLore().toString().toLowerCase().contains("stealable")) {
                stealables[i] = itemToSteal;
                inv.setItem(i, null);
            }
        }
    ...
    private void setBodySpellEffects(Player p, Body b) {
        //World effects
        if(p.hasPotionEffect(PotionEffectType.HUNGER)) b.setDiseased(true);

        //If player is a Fighter, no need to check inventory for spells
        if(PlayerClass.getPlayerClass(p).equals("Fighter")) return;

        //If player is a Caster, filter through inventory for appropriate spells
